...
class Metric_mIoU():
    ...
    def count_miou(self):
        ...
        for ind_class in range(self.num_classes-1):
            ...
            dict_iou.update({self.class_names[ind_class]: round(mIoU[ind_class] * 100, 2)})
        print(f'===> mIoU of {self.cnt} samples: ' + str(round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)))
        dict_iou.update({'mIoU': round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)})
...
class Metric_mIoU_surround():
    ...
    #     # Calculate overall IoU (intersection over union for all classes combined)
    #     total_intersection = np.diag(self.hist).sum()
    #     total_union = self.hist.sum(1).sum() + self.hist.sum(0).sum() - np.diag(self.hist).sum()
    ...
    #     print(f'===> IoU of {self.cnt} samples: ' + str(round(overall_iou * 100, 2)))
    #     print(f'===> mIoU of {self.cnt} samples: ' + str(round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)))
        
    #     dict_iou.update({'IoU': round(overall_iou * 100, 2)})
    #     dict_iou.update({'mIoU': round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)})
    #     # print(f'===> sample-wise averaged mIoU of {cnt} samples: ' + str(round(np.nanmean(mIoU_avg), 2)))
    ...
    def count_miou(self):
        ...
        # Calculate IoU for occupied voxels - Scene Completion task
        # Following the paper's definition: IoU for occupied voxels ignoring semantic class
        freespace_class = 16  # 'free' class index
        
        # Calculate TP, FP, FN for occupied voxels IoU (not freespace)
        tp_occupied = self.hist[:freespace_class, :freespace_class].sum()  # True Positive for occupied (any non-free class predicted correctly)
        fp_occupied = self.hist[freespace_class, :freespace_class].sum()  # False Positive (predicted as occupied but actually freespace)
        fn_occupied = self.hist[:freespace_class, freespace_class].sum()  # False Negative (predicted as freespace but actually occupied)
        
        union_occupied = tp_occupied + fp_occupied + fn_occupied
        iou_occupied = tp_occupied / union_occupied if union_occupied > 0 else 0.0
        ...
        print(f'===> Occupied IoU of {self.cnt} samples: ' + str(round(iou_occupied * 100, 2)))
        print(f'===> mIoU of {self.cnt} samples: ' + str(round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)))
        
        dict_iou.update({'Occupied_IoU': round(iou_occupied * 100, 2)})
        dict_iou.update({'mIoU': round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)})
        # print(f'===> sample-wise averaged mIoU of {cnt} samples: ' + str(round(np.nanmean(mIoU_avg), 2)))
