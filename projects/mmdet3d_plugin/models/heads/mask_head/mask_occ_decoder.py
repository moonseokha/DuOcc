...
class MaskOccDecoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        super(MaskOccDecoder, self).__init__(*args, **kwargs)
    ...
    def forward(self,
        ...
        intermediate_sampling_locations = []
        for lid, layer in enumerate(self.layers):
            # we just stack tpv_hw, tpv_zh, tpv_wz into 3 levels
            # rp_hw, rp_zh, rp_wz = reference_points
            # reference_points_input = reference_points[..., :2].unsqueeze(
            #     2)  # BS NUM_QUERY NUM_LEVEL 2
            # reference_points_input = torch.stack((rp_hw[...,:2], rp_zh[...,:2], rp_wz[...,:2]),
            #                                       2) # 3 BS NUM_QUERY NUM_LEVEL 2
            output = layer(
                output,
                *args,
...
class MaskOccDecoderLayer(BaseModule):
