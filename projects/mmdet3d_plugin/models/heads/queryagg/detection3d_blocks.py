...
class SparseBox3DKeyPointsGenerator(BaseModule):
    def __init__(
        ...
        num_learnable_pts=0,
        fix_scale=None,
        learnable_pts_all_num=None,
    ):
        super(SparseBox3DKeyPointsGenerator, self).__init__()
        self.embed_dims = embed_dims
        if learnable_pts_all_num is not None:
            num_learnable_pts = learnable_pts_all_num
    ...
    def anchor_projection(
        ...
        for i in range(len(T_src2dst_list)):
            ...
            # TODO: Fix bug
            index = [X, Y, Z, W, L, H, COS_YAW, SIN_YAW] + [VX, VY, VZ][:vel_dim] # [0,1,2,3,4,5,7,6]
