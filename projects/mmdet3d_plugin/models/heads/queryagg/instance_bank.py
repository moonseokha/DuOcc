...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        if (
            ...
            for i,m in enumerate(self.mask):
                if m == False:
                    self.cached_feature[i] = self.cached_feature[i].new_zeros(self.cached_feature[i].shape)
                    self.cached_anchor[i] = self.cached_anchor[i].new_zeros(self.cached_anchor[i].shape)
                    # self.cached_vox_feature[i] = self.cached_vox_feature[i].new_zeros(self.cached_vox_feature[i].shape)
                    self.confidence[i] = self.confidence[i].new_zeros(self.confidence[i].shape)
                    if self.instance_id is not None:
                        self.instance_id[i]= self.instance_id[i].new_ones(self.instance_id[i].shape) * (-1)
                    # print("reset")
            ...
            if self.anchor_handler is not None:
                T_temp2cur = self.cached_anchor.new_tensor(
                    np.stack(
                        [
                            x["T_global_inv"]  # global to ego
                            @ self.metas["img_metas"][i]["T_global"] # ego to global
    ...
    def update_instance_id(self, instance_id=None, confidence=None):
        ...
        instance_id = instance_id.detach()
