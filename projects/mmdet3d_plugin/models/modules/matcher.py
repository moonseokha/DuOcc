# modified from https://github.com/Atten4Vis/MS-DETR/blob/main/models/matcher_o2m.py
# ---------------------------------------------
...
class Matcher(object):
    ...
    def __init__(
        self, thresholds: List[float], labels: List[int], allow_low_quality_matches: bool = True
    ):
        """
        Args:
...
class Stage2Assigner(nn.Module):
    def __init__(self, threshold=0.4, k=6, coef_box=0.7, coef_cls=0.3,allow_low_quality_matches=False,class_score_thr=0.3):
        super().__init__()
        self.positive_fraction = 0.25
        self.bg_label = 10  # number > 91 to filter out later
        # self.batch_size_per_image = num_queries
        self.k = k
        self.coef_box = coef_box
        self.coef_cls = coef_cls
        self.class_score_thr = class_score_thr
        self.proposal_matcher = Matcher(thresholds=[threshold], labels=[0, 1], allow_low_quality_matches=allow_low_quality_matches)
