...
__all__ = ["LSSViewTransformer","LSSViewTransformerBEVDepth"]
...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, cam2coor, cam2imgs, post_rots, post_trans,
                       bda, cam2lidar, projection_mat=None):
        """Calculate the locations of the frustum points in the lidar
        coordinate system.

        Args:
            cam2coor  : cam2coor transformation matrix in shape (B,N,4,4)
            ...
            bda (torch.Tensor): BDA transformation matrix in shape (B, N_cams,
                3, 3).
            lidar2img (torch.Tensor, optional): Lidar to image transformation in shape
                (B, N_cams, 4, 4). Defaults to None.
        Returns:
            torch.tensor: Point coordinates in shape
                (B, N_cams, D, ownsample, 3)
        """
        B, N, _, _ = cam2coor.shape

        # post-transformation
        # B x N x D x H x W x 3

        if self.surround_occ:
            points = self.frustum.to(cam2lidar) - post_trans.view(B, N, 1, 1, 1, 3)
            ...
            points = self.frustum.to(cam2coor) - post_trans.view(B, N, 1, 1, 1, 3)
            points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1))

            # cam_to_ego
            points = torch.cat((points[..., :2, :] * points[..., 2:3, :], points[..., 2:3, :]), 5)
            combine = cam2coor[:,:,:3,:3].matmul(torch.inverse(cam2imgs))
            points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
            points += cam2coor[:,:,:3, 3].view(B, N, 1, 1, 1, 3)
        points = bda[:, :3, :3].view(B, 1, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1)).squeeze(-1)
    ...
    def voxel_pooling_v2(self,ranks_bev, ranks_depth, ranks_feat,interval_starts, interval_lengths, coor, depth, feat):
    ...
    def pre_compute(self, input):
        if self.initial_flag:
            coor = self.get_lidar_coor(*input[1:8])
            self.init_acceleration_v2(coor)
            self.initial_flag = False

    def view_transform_core(self, input, depth, tran_feat,projection_mat=None):
        B, N, C, H, W = input[0].shape

        # Lift-Splat
        if self.accelerate:
            feat = tran_feat.view(B, N, self.out_channels, H, W)
            feat = feat.permute(0, 1, 3, 4, 2) # [B, N, H, W, C]
            ...
            coor = self.get_lidar_coor(*input[1:8],projection_mat=projection_mat)
            ...
            bev_feat = self.voxel_pooling_v2(ranks_bev, ranks_depth, ranks_feat,interval_starts, interval_lengths,
                coor, depth.view(B, N, self.D, H, W),
                tran_feat.view(B, N, self.out_channels, H, W))
        return bev_feat, depth

    def view_transform(self, input, depth, tran_feat,projection_mat=None):
        if self.accelerate:
            self.pre_compute(input)
        return self.view_transform_core(input, depth, tran_feat,projection_mat=projection_mat)
...
class DepthNet(nn.Module):
    ...
    @force_fp32(apply_to=('x', 'mlp_input'))
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    #     self.depth_net.apply(init_weight)
                
    def get_mlp_input(self,view_tran_components):
        cam2ego, intrin, post_rot, post_tran, bda, cam2lidar = view_tran_components
        ...
        mlp_input = torch.stack([
            ...
            bda[:, :, 2, 2],], dim=-1)
        if self.surround_occ:
            proj_mat = cam2lidar[:,:,:3,:].reshape(B, N, -1)
        else:
            proj_mat = cam2ego[:,:,:3,:].reshape(B, N, -1)
        mlp_input = torch.cat([mlp_input, proj_mat], dim=-1)
        return mlp_input
    ...
    @force_fp32(apply_to=('depth_labels', 'depth_preds'))
    ...
    @force_fp32(apply_to=('bev_feat', 'depth'))
    def forward(self, input,projection_mat,mlp_input, stereo_metas=None):
        (x, sensor2ego, intrins, post_rots, post_trans, bda,cam2lidar) = input
        B, N, C, H, W = x.shape
        x = x.view(B * N, C, H, W)
        ...
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        # clamp the depth to [0,1] to avoid nan loss but keep the gradient
        depth = torch.clamp(depth, 1e-14, 1.0)
        bev_feat, depth = self.view_transform(input, depth, tran_feat,projection_mat=projection_mat)
        return bev_feat, depth
